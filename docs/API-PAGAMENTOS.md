# API de Pagamentos Getfy

Documentação da API que permite plataformas e SaaS externos processarem pagamentos usando os gateways já configurados na Getfy (Efí, Stripe, Spacepag, etc.). A API atua como **roteador de pagamentos**: você autentica com uma **Aplicação** (API key), escolhe o modo de checkout e os pagamentos são processados com a ordem e redundância de gateways definidas por aplicação.

---

## Início rápido

- **Base URL:** Todas as rotas estão sob `/api/v1`. Exemplo: `https://sua-instancia.getfy.com/api/v1`.
...
Detalhes em [Autenticação](#2-autenticacao) e nas seções de cada endpoint abaixo.
...
1. No painel Getfy, acesse **API Pagamentos** (ou **Aplicações**).
2. Crie uma nova aplicação ou edite uma existente.
3. Na criação, a API key é exibida **uma única vez**; copie e guarde em local seguro.
4. Em edição, use **Gerar nova API key** se precisar de uma nova (a anterior deixa de funcionar).
...
Cada **Aplicação** representa um cliente da API (ex.: sua loja, seu SaaS). Nela você configura:

- **Nome** e identificação (slug).
- **Gateways por método**: PIX, cartão, boleto (e opcionalmente PIX automático, cripto), com **redundância** (ordem de fallback), da mesma forma que nos produtos Getfy.
...
| `return_url` | string | Não | URL para redirecionar após conclusão |
...
{
  "session_id": "123",
  "checkout_url": "https://sua-instancia.getfy.com/api-checkout/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
  "expires_at": "2026-03-09T12:30:00.000000Z"
}
```

- **checkout_url**: link que o usuário final deve abrir. Na página, ele verá valor e método de pagamento (PIX, boleto); não preenche dados de cliente.
- A sessão expira no horário indicado em `expires_at`.

#### Fluxo do usuário final

1. Sua plataforma redireciona o cliente para `checkout_url` ou abre em nova aba.
2. Na Getfy o cliente vê valor, produto (se houver) e escolhe PIX ou boleto.
3. Ao concluir (ou gerar PIX/boleto), é redirecionado para a página de confirmação ou para `return_url` se informada.

---

## 5. Checkout Transparente

Você chama a API para criar o pagamento e exibe os dados (QR PIX, boleto, resultado do cartão) na sua interface.

### Base URL

Todos os endpoints abaixo são **POST** ou **GET** em `/api/v1/...` e exigem autenticação (Bearer ou X-API-Key).

### Dados comuns (customer)

Em todos os endpoints de criação de pagamento, use o objeto `customer` e os campos abaixo. Os endpoints PIX, cartão e boleto referenciam estes dados comuns.
...
Campos opcionais comuns em todos: `amount`, `currency`, `product_id`, `product_offer_id`, `subscription_plan_id`, `metadata`, `idempotency_key` (ou header `Idempotency-Key`).

---

### POST /api/v1/payments/pix

Cria um pedido e uma cobrança PIX. Retorna QR code e copia e cola. Os campos de cliente seguem os [dados comuns (customer)](#dados-comuns-customer) acima.
...
Use `qrcode` (imagem) ou `copy_paste` (código PIX) na sua UI. O status do pagamento pode ser consultado em **GET /api/v1/payments/{order_id}** ou via webhook.

---

### POST /api/v1/payments/card

Cria um pedido e processa o pagamento com cartão. Campos de cliente e valor seguem os [dados comuns](#dados-comuns-customer).
...
Cria um pedido e gera um boleto. Campos de cliente seguem os [dados comuns](#dados-comuns-customer).
...
- Envie no body: `"idempotency_key": "seu-uuid-ou-string-unica"`  
  ou no header: `Idempotency-Key: seu-uuid-ou-string-unica`
- Máximo 128 caracteres.
- Para a **mesma aplicação** e **mesma chave**, a API retorna a mesma resposta (cache por até 24h) sem criar novo pedido.

Recomendado em todos os endpoints de criação de pagamento (PIX, cartão, boleto).
...
- **X-Getfy-Signature**: HMAC-SHA256 do **body bruto** (string JSON) usando o webhook secret como chave.

**Como validar no seu servidor:**

1. Ler o body bruto da requisição (antes de parsear JSON).
2. Calcular `HMAC-SHA256(body_bruto, webhook_secret)`.
3. Comparar com o valor do header `X-Getfy-Signature` (comparação constante para evitar timing attacks).
...
1. **Idempotency key**: use em todas as criações de pagamento (PIX, cartão, boleto) para evitar cobranças duplicadas em retentativas.
2. **Webhook**: configure `webhook_url` e `webhook_secret`; valide sempre o header `X-Getfy-Signature` quando o secret estiver definido.
3. **API key**: nunca inclua a API key em código frontend ou em repositórios; use variáveis de ambiente no backend.
4. **HTTPS**: em produção, utilize apenas HTTPS.
5. **Tratamento de erros**: trate 4xx e 5xx e implemente retry com backoff para 5xx e 429 (respeitando o rate limit).
6. **Logs**: não registre a API key em logs; use apenas identificadores da aplicação (ex.: id ou nome).
...
Base URL: a raiz da sua instalação Getfy (ex.: `https://sua-instancia.getfy.com`). Todas as rotas da API estão sob o prefixo `/api/v1`.
