<!-- 
 * Página para a qual o cliente é redirecionado após realizar o pagamento
 * Nesse página ele aguarda até que o pagamento seja concluido através de notificação via webhook
 * Nessa página existe um script ajax para executar o arquivo `/verificar_pagamento.php` para verificar o status do pagamento no banco de dados
-->
...
<body>
    ...
    <div class="page-container">
        <?php
        /**
         * Há duas formas de receber os retornos de falha na iniciação do pagamento: por meio de parâmetros via GET ou pelo POST enviado ao webhook cadastrado. 
         * Este trecho trata os erros caso o retorno seja recebido por parâmetros via GET.
         */

        // Parâmetro identificador do pagamento
        $identificadorPagamento = $_GET['identificadorPagamento'] ?? null;

        // Captura os possíveis erros recebidos por GET
        $errors = isset($_GET['erro']) ? [
            'erro' => $_GET['erro'],
            'nome' => $_GET['nome'],
        ...
        if ($errors): ?>
            <div class="container">
                <div style="margin-top: 150px;" class="row justify-content-md-center">
                    <div class="col-md-auto text-start ">
                        <div class="d-flex justify-content-start mb-3 align-items-center">
                            <h3>O pagamento falhou 🙁</h3>
                        </div>
                        <p>Houve uma falha no pagamento. Mas não se preocupe, tente novamente.</p>
                        <hr>
                        <div class="row">
                            <div class="col-2">
                                <label class="form-label"><strong>Erro:</strong></label>
                                <input class="form-control" type="text" value="<?= $errors['erro'] ?>" disabled>
                            </div>
                            <div class="col-3">
                                <label class="form-label"><strong>Nome erro:</strong></label>
                                <input class="form-control" type="text" value="<?= $errors['nome'] ?>" disabled>
                            </div>
                            <div class="col-7">
                                <label class="form-label"><strong>Mensagem:</strong></label>
                                <input class="form-control" type="text" value="<?= $errors['mensagem'] ?>" disabled>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            <script>
                /**
                 * Este script verifica se o pagamento foi concluido e redireciona para a tela de conclusão
                 */
                const redirectUrl = "pagamento_concluido.php";
                const identificadorPagamento = "<?= $identificadorPagamento ?>";

                /**
                 * Função para redirecionar para outra URL
                 */
                function urlRedirect(options) {
                    const form = $('<form/>', {
                        action: options.url,
                    ...
                    function appendField(name, value) {
                        if (Array.isArray(value)) {
                            // Se for um array, iterar sobre os valores e criar inputs com o mesmo nome
                            value.forEach((val, index) => {
                                appendField(`${name}[${index}]`, val);
                            });
                        } else if (typeof value === 'object' && value !== null) {
                            // Se for um objeto, iterar sobre as chaves e criar inputs
                            $.each(value, function (key, val) {
                                appendField(`${name}[${key}]`, val);
                            });
                        } else {
                            // Se for um valor simples, criar o input
                            form.append($('<input>', {
                                type: 'hidden',
                                name: name,
                                value: value
                 ...
                 * Executa o arquivo "verificar_pagamento.php" a cada 3 segundos, e ele consulta o histórico de pagamentos para verificar o status do pagamento
                 */
                function pollPaymentStatus() {
                    setTimeout(function () {
                        $.ajax({
                            type: "POST",
                            url: "verificar_pagamento.php",
                            data: {
                            ...
                            success: function (response) {
                                ...
                                if (!response) {
                                    ...
                                    if (status === 'pendente') {
                                        pollPaymentStatus();
                                    } else if (status === 'inexistente') {
                                        $('#conteudo').html('<h2>Identificador de pagamento inexistente</h2>');
                                    } else {
                                        urlRedirect({
                                            url: redirectUrl,
                                            method: "POST",
                ...
                /**
                 * Se o pagamento não acontecer em 6 minutos, entende-se que a cobrança expirou, então redireciona para o pagamento concluído com status "expirado"
                 */
                setTimeout(function () {
                    urlRedirect({
                        url: redirectUrl,
            ...
            <div class="container">
                <div style="margin-top: 150px;" class="row justify-content-md-center">
                    <div class="col-md-auto text-start" id="conteudo">
                        <div class="d-flex justify-content-start mb-3 align-items-center">
                            <h3>Processando o pagamento</h3>
                            <div class="spinner-border ms-2" style="width: 30px; height: 30px; color: #00809d;"
                                role="status">
                                <span class="visually-hidden">Loading...</span>
                            </div>
                        </div>
                        <p>Assim que o pagamento for processado, atualizaremos esta página confirmando! 😍</p>
                    </div>
                </div>
            </div>
        <?php endif; ?>
