...
### HTTPS automático na VPS (Let's Encrypt + Cloudflare Full)

Se você rodar o instalador **em SSH interativo** (terminal ligado ao teclado), **no início** ele **pergunta o domínio** e, se preencher, o **e-mail para Let's Encrypt**. Se deixar o domínio em branco (Enter), segue só com HTTP (útil para testar por IP).

Ou defina domínio e e-mail **por variáveis de ambiente** (útil para pipes/automação):

```bash
GETFY_DOMAIN=loja.seudominio.com \
GETFY_ACME_EMAIL=voce@seudominio.com \
bash -c "$(curl -fsSL https://raw.githubusercontent.com/LeonardoIsrael0516/lucas/main/install.sh)"
```

No **Cloudflare**, use SSL/TLS **Full** ou **Full (strict)** e mantenha **80** e **443** abertos na firewall da VPS. Detalhes: [docs/INSTALL_VPS.md](docs/INSTALL_VPS.md).
...
No modo **HTTPS automático** (`GETFY_DOMAIN` + `GETFY_ACME_EMAIL`), as portas da origem são **80** e **443** (requisito típico do ACME e do modo Full).
