...
class ProdutosController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $query->withCount(['users as alunos_count'])
            ->selectSub(
                DB::table('orders')
                    ->selectRaw('count(*)')
    ...
    public function duplicate(Product $produto)
    {
        ...
        // Copy product image (best-effort)
        if ($produto->image) {
            $copied = $this->copyStorageAssetBestEffort($storage, $produto->image, 'products/'.$newProduct->id);
            if ($copied) {
                $newProduct->image = $copied;
                $newProduct->save();
            }
        }
                ...
                foreach ($s->modules as $m) {
                    ...
                    foreach ($m->lessons as $l) {
                        ...
                        if (is_string($newContentUrl) && $newContentUrl !== '') {
                            $copiedUrl = $this->copyStorageUrlBestEffort($storage, $newContentUrl, 'member-area/'.$newProduct->id);
                            if ($copiedUrl) {
                        ...
                        if (is_array($newFiles) && $newFiles !== []) {
                            ...
                            foreach ($newFiles as $it) {
                                ...
                                $copiedUrl = $url !== '' ? $this->copyStorageUrlBestEffort($storage, $url, 'member-area/'.$newProduct->id) : null;
                                $out[] = [
                                    'url' => $copiedUrl ?: $url,
                                    'name' => $name,
                                ];
    ...
    private function storagePathFromValue(string $value): ?string
    {
        ...
        if (str_starts_with($v, 'member-area/') || str_starts_with($v, 'products/') || str_starts_with($v, 'branding/') || str_starts_with($v, 'email-templates/')) {
            return $v;
        }
    ...
    private function productToArray(Product $p, array $rates): array
    {
        ...
        return [
            ...
            'alunos_count' => (int) ($p->alunos_count ?? 0),
