...
class MemberBuilderController extends Controller
{
    ...
    private function copyStorageAssetBestEffort(StorageService $storage, string $value, string $destDir): ?string
    {
        $srcPath = $this->storagePathFromValue($value);
        if (! $srcPath || ! $storage->disk()->exists($srcPath)) {
            return null;
        }
        ...
        try {
            $ok = $storage->disk()->copy($srcPath, $destPath);
    ...
    private function copyStorageUrlBestEffort(StorageService $storage, string $urlOrPath, string $destDir): ?string
    {
        $srcPath = $this->storagePathFromValue($urlOrPath);
        if (! $srcPath) return null;

        $destPath = $this->copyStorageAssetBestEffort($storage, $srcPath, $destDir);
        return $destPath ? $storage->url($destPath) : null;
    }
    ...
    public function duplicateModule(Request $request, Product $produto, MemberModule $module): JsonResponse
    {
        $this->authorizeProduct($produto);
        if ($module->product_id !== $produto->id) {
            abort(404);
        ...
        if (is_string($thumb) && trim($thumb) !== '') {
            $copied = $this->copyStorageAssetBestEffort($storage, $thumb, 'member-area/'.$produto->id);
            if ($copied) {
                $thumb = $copied;
            }
        }
        ...
        $newModule = MemberModule::create([
            ...
            'source_member_module_id' => $module->source_member_module_id,
            'access_type' => $module->access_type,
            'external_url' => $module->external_url,
        ]);

        foreach ($module->lessons as $l) {
            $newContentUrl = $l->content_url;
            if (is_string($newContentUrl) && $newContentUrl !== '') {
                $copiedUrl = $this->copyStorageUrlBestEffort($storage, $newContentUrl, 'member-area/'.$produto->id);
                if ($copiedUrl) {
                    $newContentUrl = $copiedUrl;
                }
            }

            $newFiles = $l->content_files;
            if (is_array($newFiles) && $newFiles !== []) {
                $out = [];
                foreach ($newFiles as $it) {
                    if (! is_array($it)) continue;
                    $url = isset($it['url']) ? (string) $it['url'] : '';
                    $name = isset($it['name']) ? (string) $it['name'] : 'Material';
                    $copiedUrl = $url !== '' ? $this->copyStorageUrlBestEffort($storage, $url, 'member-area/'.$produto->id) : null;
        ...
        $newModule->loadMissing('lessons');
        $payload = $this->moduleJsonPayloadForStore($newModule);
        $payload['lessons'] = $newModule->lessons->map(fn (MemberLesson $l) => [
            'id' => $l->id,
            'title' => $l->title,
            'position' => $l->position,
            'type' => $l->type,
        ...
        return response()->json(['message' => 'Módulo duplicado.', 'module' => $payload]);
    }

    public function duplicateLesson(Request $request, Product $produto, MemberLesson $lesson): JsonResponse
    {
        $this->authorizeProduct($produto);
        if ($lesson->product_id !== $produto->id) {
            abort(404);
        ...
        $max = MemberLesson::where('member_module_id', $lesson->member_module_id)->max('position') ?? 0;
        $storage = app(StorageService::class);

        $newContentUrl = $lesson->content_url;
        if (is_string($newContentUrl) && $newContentUrl !== '') {
            $copiedUrl = $this->copyStorageUrlBestEffort($storage, $newContentUrl, 'member-area/'.$produto->id);
            if ($copiedUrl) {
                $newContentUrl = $copiedUrl;
        ...
        $newLesson = MemberLesson::create([
            'member_module_id' => $lesson->member_module_id,
            'product_id' => $produto->id,
            'title' => ($lesson->title ?? 'Aula').' (cópia)',
            'position' => $max + 1,
            'type' => $lesson->type,
            'content_url' => $newContentUrl,
