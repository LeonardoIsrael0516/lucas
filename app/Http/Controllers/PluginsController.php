...
class PluginsController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $pluginsList = array_values(array_map(function ($p) {
            if (($p['hidden'] ?? false) === true) {
                return null;
            }
            $bannerUrl = null;
            if (! empty($p['banner'])) {
        ...
        $pluginsList = array_values(array_filter($pluginsList, fn ($row) => is_array($row)));
        ...
        $installed = array_values(array_filter($plugins, fn ($p) => ! empty($p['is_registered']) && empty($p['hidden'])));
