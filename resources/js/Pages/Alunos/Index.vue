...
const novoAlunoTab = ref('individual'); // 'individual' | 'mass'
const productFilterOpen = ref(false);
const novoAlunoForm = ref({
    name: '',
    email: '',
    phone: '',
    password_mode: 'auto', // 'auto' | 'manual'
    password: '',
    product_ids: [],
    send_access_email: true,
    send_access_whatsapp: false,
});
const savingNovo = ref(false);
const importForm = ref({ file: null, product_ids: [], send_access_email: true, send_access_whatsapp: false });
...
const novoAlunoProductSearch = ref('');
const importProductSearch = ref('');

const filteredProdutosForNovo = computed(() => {
    const q = (novoAlunoProductSearch.value ?? '').trim().toLowerCase();
    if (!q) return props.produtos;
    return props.produtos.filter((p) => (p.name ?? '').toLowerCase().includes(q));
});

const filteredProdutosForImport = computed(() => {
    const q = (importProductSearch.value ?? '').trim().toLowerCase();
    if (!q) return props.produtos;
    return props.produtos.filter((p) => (p.name ?? '').toLowerCase().includes(q));
});
...
function openNovoAluno() {
    novoAlunoTab.value = 'individual';
    novoAlunoProductSearch.value = '';
    importProductSearch.value = '';
    novoAlunoForm.value = {
        name: '',
        email: '',
        phone: '',
...
function openImportModal() {
    novoAlunoTab.value = 'mass';
    novoAlunoProductSearch.value = '';
    importProductSearch.value = '';
    importForm.value = { file: null, product_ids: [], send_access_email: true, send_access_whatsapp: false };
    novoAlunoModalOpen.value = true;
}
...
async function saveImport() {
    if (!importForm.value.file) {
        showToast('Selecione um arquivo CSV ou Excel.', 'error');
        return;
    }
    ...
    try {
        ...
        formData.append('send_access_whatsapp', importForm.value.send_access_whatsapp ? '1' : '0');
...
async function saveNovoAluno() {
    ...
    try {
        const { data } = await axios.post('/produtos/alunos', {
            ...
            send_access_whatsapp: novoAlunoForm.value.send_access_whatsapp ?? false,
        });
    ...
    <div class="space-y-6">
        ...
        <Teleport to="body">
            ...
            >
                ...
                />
                <div
                    class="relative w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900"
                >
                    <div class="mb-5 flex items-start justify-between gap-3">
                        <div>
                            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Cadastrar novo aluno</h3>
                            <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Cadastro individual ou importação em massa.</p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="mb-5 grid grid-cols-2 gap-2 rounded-xl bg-zinc-100 p-1 dark:bg-zinc-800/60">
                        <button
                            type="button"
                            class="rounded-lg px-3 py-2 text-sm font-medium transition"
                            :class="novoAlunoTab === 'individual' ? 'bg-white text-zinc-900 shadow dark:bg-zinc-900 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'"
                            @click="novoAlunoTab = 'individual'"
                        >
                            Cadastro individual
                        </button>
                        <button
                            type="button"
                            class="rounded-lg px-3 py-2 text-sm font-medium transition"
                            :class="novoAlunoTab === 'mass' ? 'bg-white text-zinc-900 shadow dark:bg-zinc-900 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'"
                            @click="novoAlunoTab = 'mass'"
                        >
                            Importação em massa
                        </button>
                    </div>

                    <div v-if="novoAlunoTab === 'individual'" class="space-y-4">
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-2 gap-2 rounded-xl bg-zinc-100 p-1 dark:bg-zinc-800/60">
                                <button
                                    type="button"
                                    class="rounded-lg px-3 py-2 text-sm font-medium transition"
                                    :class="novoAlunoForm.password_mode === 'auto' ? 'bg-white text-zinc-900 shadow dark:bg-zinc-900 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'"
                                    @click="novoAlunoForm.password_mode = 'auto'"
                                >
                                    Automática
                                </button>
                                <button
                                    type="button"
                                    class="rounded-lg px-3 py-2 text-sm font-medium transition"
                                    :class="novoAlunoForm.password_mode === 'manual' ? 'bg-white text-zinc-900 shadow dark:bg-zinc-900 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'"
                                    @click="novoAlunoForm.password_mode = 'manual'"
                                >
                                    Manual
                                </button>
                            </div>
                            <input
                                v-if="novoAlunoForm.password_mode === 'manual'"
                        ...
                        <div class="space-y-2">
                            <label class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left">
                                <span class="shrink-0 w-fit">
                                    <Checkbox :model-value="novoAlunoForm.send_access_whatsapp" @update:model-value="novoAlunoForm.send_access_whatsapp = $event" />
                                </span>
                                <span class="flex-1 text-left text-sm text-zinc-900 dark:text-white">Enviar WhatsApp de acesso ao criar</span>
                            </label>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input
                                    v-model="novoAlunoProductSearch"
                                    type="text"
                                    class="mb-2 w-full rounded-lg border border-zinc-200 bg-white pl-9 pr-3 py-2 text-sm text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"
                                    placeholder="Buscar cursos..."
                                />
                            </div>
                            <div class="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                                <label
                                    v-for="p in filteredProdutosForNovo"
                                    :key="p.id"
                    ...
                    <div v-else class="space-y-4">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2">
                            <label class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left">
                                ...
                                <span class="flex-1 text-left text-sm text-zinc-900 dark:text-white">Enviar e-mail de acesso</span>
                            </label>
                            <label class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left">
                                <span class="shrink-0 w-fit">
                                    <Checkbox :model-value="importForm.send_access_whatsapp" @update:model-value="importForm.send_access_whatsapp = $event" />
                                </span>
                                <span class="flex-1 text-left text-sm text-zinc-900 dark:text-white">Enviar WhatsApp de acesso</span>
                            </label>
                        </div>
                        <div class="space-y-2">
                            <p class="text-xs font-medium uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                            ...
                            <div class="max-h-56 space-y-1 overflow-y-auto rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                                <label
                                    v-for="p in filteredProdutosForImport"
                                    :key="p.id"
                                    class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
                                >
                                    <span class="shrink-0 w-fit">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <Button variant="outline" :disabled="savingNovo || importing" @click="closeNovoAluno">
                            Cancelar
                        </Button>
                        <Button
                            v-if="novoAlunoTab === 'individual'"
                            variant="primary"
                            :disabled="savingNovo"
                            @click="saveNovoAluno"
                        >
