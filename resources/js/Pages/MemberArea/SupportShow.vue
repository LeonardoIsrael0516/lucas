...
import { ArrowLeft, Bell, LayoutGrid, LifeBuoy, LogOut, MessageCircle, UserRound } from 'lucide-vue-next';
...
function sendReply() {
    replyForm.post(`/suporte/${props.ticket.id}/reply`, {
        preserveScroll: true,
        ...
        <div class="relative flex min-w-0 flex-1 flex-col">
            <header class="sticky top-0 z-20 border-b border-zinc-200 bg-white/95 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/95">
                <div class="flex h-14 items-center justify-between gap-3 px-4 md:px-6">
                    <div class="flex min-w-0 items-center gap-3">
                        <Link href="/suporte" class="inline-flex shrink-0 items-center gap-1 rounded-lg p-2 text-zinc-600 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800">
            ...
            <main class="flex-1 px-4 py-8 md:px-8">
                ...
                <div class="mb-4 flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-xs text-zinc-500">{{ fmtDate(ticket.created_at) }}</span>
                </div>

                <div class="space-y-4 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
                    <div v-for="m in ticket.messages" :key="m.id" class="flex flex-col gap-1">
                        <div class="flex flex-wrap items-baseline gap-2">
                            ...
                            <span
                                v-if="m.is_staff"
                                class="rounded bg-sky-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-800 dark:bg-sky-900/40 dark:text-sky-200"
                                >Equipe</span
                            >
                        </div>
                        <p class="whitespace-pre-wrap text-sm text-zinc-700 dark:text-zinc-300">{{ m.body }}</p>
                    </div>
                </div>

                <form v-if="isOpen" class="mt-6 space-y-3" @submit.prevent="sendReply">
                    <label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400">Nova mensagem</label>
                    <textarea
                    ...
                    <button
                        ...
                        class="rounded-lg px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60"
                        :style="{ backgroundColor: 'var(--student-primary)' }"
                    >
                        Enviar
                    </button>
                </form>
                <p v-else class="mt-6 text-sm text-zinc-500 dark:text-zinc-400">Este ticket foi encerrado. Você pode ver o histórico acima.</p>
            </main>
